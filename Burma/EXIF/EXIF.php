...
function Interpret_EXIF_to_HTML( $Exif_array, $filename )
{
        ...
        //ADDED (AMIT)
        $output_str .= "<tr class=\"EXIF_Table_Row\"><td class=\"EXIF_Caption_Cell\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Image Name</td><td class=\"EXIF_Value_Cell\">" . getAtomicImageName($filename) . "</td></tr>\n";
...
function interpret_IFD( $IFD_array, $filename )
{
        ...
        foreach( $IFD_array as $Tag_ID => $Exif_Tag )
        {
                ...
                else if ( $Exif_Tag['Decoded'] == TRUE )
                {
                        ...
                        {
                                //ADDED (AMIT)
                                //I want only the exposure, aparture and ISO
                                if($Exif_Tag['Tag Name'] == 'Exposure Time' || $Exif_Tag['Tag Name'] == 'Aperture F Number' || $Exif_Tag['Tag Name'] == 'ISO Speed Ratings')
                                {
                                	if($Exif_Tag['Tag Name'] == 'Aperture F Number')
                                	{
                                		$output_str .= "<tr class=\"EXIF_Table_Row\"><td class=\"EXIF_Caption_Cell\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . $Exif_Tag['Tag Name'] . "</td><td class=\"EXIF_Value_Cell\">" . parseAparture($Exif_Tag['Text Value']) . "</td></tr>\n";
                                	}
                                	else if($Exif_Tag['Tag Name'] == 'Exposure Time')
									                                	{
									    $output_str .= "<tr class=\"EXIF_Table_Row\"><td class=\"EXIF_Caption_Cell\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . $Exif_Tag['Tag Name'] . "</td><td class=\"EXIF_Value_Cell\">" . parseExposure($Exif_Tag['Text Value']) . "</td></tr>\n";
                                	}
                                	else
                                	{
                        ...
                        {
                                //ADDED (AMIT)
                                if($Exif_Tag['Tag Name'] == 'Image Description' || $Exif_Tag['Tag Name'] == 'Date and Time of Original' || $Exif_Tag['Tag Name'] == 'Flash' || $Exif_Tag['Tag Name'] == 'Make (Manufacturer)' || $Exif_Tag['Tag Name'] == 'Model')
                                {
                                	// Other tag - Output text as preformatted
                                	if($Exif_Tag['Tag Name'] == 'Date and Time of Original')
                                	{
                                		$output_str .= "<tr class=\"EXIF_Table_Row\"><td class=\"EXIF_Caption_Cell\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . $Exif_Tag['Tag Name'] . "</td><td class=\"EXIF_Value_Cell\">" . convertDate(trim( $Exif_Tag['Text Value'])) . "</td></tr>\n";
                                	}
                                	else
                                	{
...
function getAtomicImageName ($filename)
{
	...
	$replacables = array("-","@");
	$atomic = str_replace($replacables, ' ', $atomic);
	return $atomic;
}

//convert date of type yyyy:mm:dd hh:mm:ss to : dd.mm.yyyy hh:mm:ss
...
function parseAparture($origApar)
{
	$parts = explode(' ' , $origApar);
	$newApar = substr($parts[1], 1, strlen($parts[1])-2);
	return $newApar;
}

function parseExposure($origExp)
{
	$expParts = explode(' ', $origExp);
	$expNumericParts = explode('/', $expParts[0]);
	$atomicValue = $expNumericParts[1] / $expNumericParts[0];
	return "1/".$atomicValue;
}
