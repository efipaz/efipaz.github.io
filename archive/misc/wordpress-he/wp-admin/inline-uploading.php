...
if ( preg_match('!^image/!', $attachment['post_mime_type']) ) {
	...
	/* WPH - Image thumbnail generation overridden 
...
imgb[{$ID}] = '<img id=\"image{$ID}\" src=\"$src\" alt=\"{$title}\"  />'; <?php /* WPH - image size bug */ ?>
imga[{$ID}] = '<img id=\"image{$ID}\" src=\"{$image['guid']}\" alt=\"{$title}\" />'; <?php /* WPH - image size bug */ ?>
";
			$html .= "<div id='target{$ID}' class='attwrap left'>
	<div id='div{$ID}' class='imagewrap' onclick=\"doPopup({$ID});\">
		<img id=\"image{$ID}\" src=\"$src\" alt=\"{$title}\" /> <?php /* WPH - image size bug */ ?>
	</div>
	{$noscript}
</div>
";
...
#images .attwrap { /* WPH - image size bug */
	height: 78px !important;
	overflow: hidden;
	}
#images img { /* WPH - image size bug */
	height: 78px;
	}
...
<div id="submit">
<input type="submit" value="<?php _e('Upload File'); ?>" /> <?php /* WPH - Identical strings used for different meanings. English phonology thing. */ ?>
<?php if ( !empty($all) ) : ?>
