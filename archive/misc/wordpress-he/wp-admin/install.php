...
if (!file_exists('../wp-config.php'))
    die("לא מצאתי את הקובץ <code>wp-config.php</code>. אני צריכה את זה לפני שנתחיל. רוצה עוד עזרה? <a href='http://wordpress.org/docs/faq/#wp-config'>פה יש הרבה</a>. אפשר ליצור קובץ <code>wp-config.php</code> <a href='setup-config.php'> עם הטופס הזה</a>, אבל זה לא עובד בכל המקרים. הדרך הבטוחה ביותר היא ליצור את הקובץ ידנית.
");
...
<head>
	...
	body {
		background: #fff;
		color: #000;
		font-family: serif; /* WPH - Hebrew font specificity */
		margin-left: 20%;
		margin-right: 20%;
		padding: .2em 2em;
		unicode-bidi: embed;
		direction: rtl; /* WPH - directionality */
	}
...
$wpdb->query("INSERT INTO $wpdb->links (link_url, link_name, link_category, link_rss, link_notes) VALUES ('http://www.trans.co.il/', 'רן', 1, 'http://www.trans.co.il/feed/', '');");
$wpdb->query("INSERT INTO $wpdb->links (link_url, link_name, link_category, link_rss, link_notes) VALUES ('http://blogs.linux.ie/xeer/', 'דונצה', 1, 'http://blogs.linux.ie/xeer/feed/', '');");
$wpdb->query("INSERT INTO $wpdb->links (link_url, link_name, link_category, link_rss, link_notes) VALUES ('http://zengun.org/weblog/', 'מישל', 1, 'http://zengun.org/weblog/feed/', '');");
$wpdb->query("INSERT INTO $wpdb->links (link_url, link_name, link_category, link_rss, link_notes) VALUES ('http://boren.nu/', 'ריאן', 1, 'http://boren.nu/feed/', '');");
$wpdb->query("INSERT INTO $wpdb->links (link_url, link_name, link_category, link_rss, link_notes) VALUES ('http://photomatt.net/', 'מאט', 1, 'http://xml.photomatt.net/feed/', '');");
$wpdb->query("INSERT INTO $wpdb->links (link_url, link_name, link_category, link_rss, link_notes) VALUES ('http://zed1.com/journalized/', 'מייק', 1, 'http://zed1.com/journalized/feed/', '');");
$wpdb->query("INSERT INTO $wpdb->links (link_url, link_name, link_category, link_rss, link_notes) VALUES ('http://www.alexking.org/', 'אלכס', 1, 'http://www.alexking.org/blog/wp-rss2.php', '');");
$wpdb->query("INSERT INTO $wpdb->links (link_url, link_name, link_category, link_rss, link_notes) VALUES ('http://dougal.gunters.org/', 'דוגל', 1, 'http://dougal.gunters.org/feed/', '');");
// WPH - Links in Hebrew 

// Default category
$wpdb->query("INSERT INTO $wpdb->categories (cat_ID, cat_name, category_nicename, category_count, category_description) VALUES ('0', '".$wpdb->escape(__('Uncategorized'))."', '".sanitize_title('General')."', '1', '')");
// WPH - category names in Hebrew 

// First post
$now = date('Y-m-d H:i:s');
$now_gmt = gmdate('Y-m-d H:i:s');
$wpdb->query("INSERT INTO $wpdb->posts (post_author, post_date, post_date_gmt, post_content, post_excerpt, post_title, post_category, post_name, post_modified, post_modified_gmt, comment_count, to_ping, pinged, post_content_filtered) VALUES ('1', '$now', '$now_gmt', '".$wpdb->escape(__('Welcome to WordPress. This is your first post. Edit or delete it, then start blogging!'))."', '', '".$wpdb->escape(__('Hello world!'))."', '0', '".$wpdb->escape('hello-world')."', '$now', '$now_gmt', '1', '', '', '')");
// WPH - First post in Hebrew 
