...
function wptexturize($text) {
	...
	for ($i = 0; $i < $stop; $i++) {
		...
		if (isset($curl{0}) && '<' != $curl{0} && $next) { // If it's not a tag
			...
			$curl = preg_replace('/(\s|\A)\'(?!\s)/', '$1&#8217;$2', $curl); /* WPH - directionality - right single quotation mark */
			$curl = preg_replace('/\'(\s|\Z|\S)/', '&#8216;$1', $curl); /* WPH - directionality - left single quotation mark */
			...
			$curl = preg_replace('/(\s|\A)"(?!\s)/', '$1&#8221;$2', $curl); /* WPH - directionality - right double quotation mark */
			$curl = preg_replace('/"(\s|\Z|\S)/', '&#8220;$1', $curl); /* WPH - directionality - left double quotation mark */
