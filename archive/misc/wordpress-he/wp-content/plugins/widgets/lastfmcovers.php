<?php
/*
Plugin Name: תקליטים מ-Last.fm
Description: ווידג'ט לבר הצדדי שמציג עטיפות של תקליטים להם האזנת לאחרונה מ-Last.fm
Author: Dog Of Dirk, תרגום על ידי רן יניב הרטשטיין
Version: 1.7
...
function widget_lastfmcovers_init() {
  ...
  function runForCover($username, $imgwidth, $maxcount, $countdesc) {
    ...
    foreach($_output as $_cd) {
    	if ($_getoond < $maxcount) {
    	  if (array_key_exists("image", $_cd)) {
    		  if (false === strpos($_cd['image'], "noalbum")) {
    	      $_alttitle = $_cd['name'] . "&nbsp;&raquo;&nbsp;" . $_cd['artist'] . "&nbsp;&raquo;&nbsp;" . str_replace('%', $_cd['playcount'], $countdesc);
    	      $_imgstyle = ($imgwidth > 0) ? ' style="width: ' . $imgwidth . 'px; height: ' . $imgwidth . 'px; padding: 8px 8px 0px 0px;border:none !important;"' : ' class="lastfmcover"';
    ...
    return ('' == $_result) ? '<li><em>אין תקליטים</em></li>' : $_result;
  }
	...
	function widget_lastfmcovers_control() {
		...
		$options = get_option('widget_lastfmcovers');
		if ( !is_array($options) ) {
			$options = array('title'=>'favorite cds', 'username'=>'', 'count' => '6', 'imgwidth' => '70', 'countdesc' => '% האזנות.');
    }
      ...
      if ($options['count'] < 1) {
		...
		echo '<p style="text-align:right;"><label for="lastfmcovers-title"><span>כותרת:</span> <input style="width: 200px;" id="lastfmcovers-title" name="lastfmcovers-title" type="text" value="'.$title.'" /></label></p>';
		echo '<p style="text-align:right;"><label for="lastfmcovers-username">שם משתמש: <input style="width: 200px;" id="lastfmcovers-username" name="lastfmcovers-username" type="text" value="'.$username.'" /></label></p>';
		echo '<p style="text-align:right;"><label for="lastfmcovers-count">כמות התקליטים המרבית שתוצג: <input style="width: 200px;" id="lastfmcovers-count" name="lastfmcovers-count" type="text" value="'.$count.'" /></label></p>';
		echo '<p style="text-align:right;"><label for="lastfmcovers-countdesc">תיאור מספר האזנות: <input style="width: 200px;" id="lastfmcovers-countdesc" name="lastfmcovers-countdesc" type="text" value="'.$countdesc.'" /></label></p>';
		echo '<p style="text-align:right;">(הסימן % יוחלף בכמות הפעמים בהם האזנת לשירים מהאלבום)</p>';
		echo '<p style="text-align:right;"><label for="lastfmcovers-imgwidth">גודל התמונה: <input style="width: 200px;" id="lastfmcovers-imgwidth" name="lastfmcovers-imgwidth" type="text" value="'.$imgwidth.'" /></label></p>';
		echo '<p style="text-align:right;">(אפשר לרשום כאן 0, ולהשתמש ב-img.lastfmcover כדי לשלוט בגודל התמונה ב-css)</p>';
		echo '<input type="hidden" id="lastfmcovers-submit" name="lastfmcovers-submit" value="1" />';
	}
	
	// This registers our widget so it appears with the other available
	// widgets and can be dragged and dropped into any active sidebars.
	register_sidebar_widget('תקליטים מ-Last.fm', 'widget_lastfmcovers');

	// This registers our optional widget control form. Because of this
	// our widget will have a button that reveals a 300x100 pixel form.
	register_widget_control('תקליטים מ-Last.fm', 'widget_lastfmcovers_control', 375, 300);
}
