<?php
/*
Plugin Name: WordPress Widgets
Plugin URI: http://svn.wp-plugins.org/widgets/trunk
Description: מאפשר התאמה אישית של הבר הצדדי עם ווידג'טים בממשק גרירה-ושחרור.
Author: Automattic, Inc.‏, התאמה לעברית על ידי רן יניב הרטשטיין
Version: 1.0
...
function register_sidebars($number = 1, $args = array()) {
	...
	$name = $args['name'] ? $args['name'] : 'בר צדדי';
...
function register_sidebar($args = array()) {
	...
	$defaults = array(
		'name' => sprintf('בר צדדי %d', count($registered_sidebars) + 1 ),
...
function sidebar_admin_setup() {
	...
	add_submenu_page('themes.php', 'ווידג\'טים לבר הצדדי', 'ווידג\'טים לבר הצדדי', 5, $page, 'sidebar_admin_page');
	if ( $_GET['page'] == $page ) {
...
?>
	...
	<h2>אודות הבר הצדדי</h2>
	<p>ערכות עיצוב מסוימות מאפשרות לשלוט במראה הבר הצדדי, להוסיף ולהסיר ממנו חלקים ולשנות את הסדר שלהם. החלקים האלה נקראים ווידג'טים, והם כוללים דברים כמו רשימות לינקים, כלים לחיפוש, מידע מאתרים אחרים, וכל דבר אחר שאפשר לשים בסיידבר. ערכת העיצוב שלך לא תומכת בווידג'טים. ברוב המקרים, אפשר <a href='http://andy.wordpress.com/widgets/get-ready'>להוסיף את התמיכה בקלות יחסית</a>. אחרת, אולי כדאי לנסות למצוא ערכת עיצוב אחרת עם תמיכה בווידג'טים )וגם תמיכה בעברית ובישור מימין למשאל(./p>
	</div>
<?php
		return;
	}
	...
	<p><?php printf('הבר הצדדי עודכן. <a href="%s">אל האתר &raquo;</a>', get_settings('home') . '/'); ?></p>
	</div>
	<?php } ?>
	<div class="wrap">
	<h2>ארגון הבר הצדדי</h2>
	<p>אפשר לגרור ווידג'טים לתוך הבר הצדדי בסדר הרצוי, והם יופיעו בבר הצדדי בבלוג.</p>
...
<?php $i = 1; foreach ( $registered_sidebars as $index => $sidebar ) : ?>
	...
	<div class="dropzone">
		...
		<div id="<?php echo $index; ?>placematt" class="module placematt"><span class="handle">
		<h4>הבר הצדדי</h4>
		<span>כשהתיבה הזו ריקה, יוצג בבלוג הבר הצדדי הרגיל שהגיע עם ערכת העיצוב. כשיש בתיבה הזו ווידג'טים, הם יוצגו בבלוג במקום הבר הצדדי הרגיל, לפי הסדר שבחרת.</span></div>
...
<?php endforeach; ?>
	<div class="dropzone">
		<h3>ווידג'טים שלא מופיעים בבר הצדדי</h3>
...
<?php endforeach; ?>
	...
	<input type="submit" value="לשמור &raquo;" />
	</p>
	<div id="controls">
<?php foreach ( $registered_widget_controls as $name => $control ) : ?>
...
function widget_description($args) {
	extract($args);
	$options = get_option('widget_description');
	$title = empty($options['title']) ? 'אודות הבלוג' : $options['title'];
	
?>
		<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
			<ul>
			<li class="blog-description no-bullet"><?php echo ( bloginfo('description') ); ?></li>
			</ul>
...
function widget_description_control() {
	$options = $newoptions = get_option('widget_description');
	if ( $_POST["description-submit"] ) {
		$newoptions['title'] = strip_tags(stripslashes($_POST["description-title"]));
	}
	if ( $options != $newoptions ) {
		$options = $newoptions;
		update_option('widget_description', $options);
	}
	$title = htmlspecialchars($options['title'], ENT_QUOTES);
?>
			<p><label for="description-title">כותרת: <input style="width: 250px;" id="description-title" name="description-title" type="text" value="<?php echo $title; ?>" /></label></p>
			<input type="hidden" id="description-submit" name="description-submit" value="1" />
<?php
}
...
function widget_pages($args) {
	...
	$title = empty($options['title']) ? 'עמודים' : $options['title'];
...
function widget_pages_control() {
	...
	$title = htmlspecialchars($options['title'], ENT_QUOTES);
?>
			<p><label for="pages-title">כותרת: <input style="width: 250px;" id="pages-title" name="pages-title" type="text" value="<?php echo $title; ?>" /></label></p>
...
function widget_search($args) {
	extract($args);
	$title = empty($options['title']) ? 'חיפוש בבלוג' : $options['title'];
?>
		<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
...
function widget_archives_control() {
	...
	$title = htmlspecialchars($options['title'], ENT_QUOTES);
?>
			<p><label for="archives-title">כותרת: <input style="width: 250px;" id="archives-title" name="archives-title" type="text" value="<?php echo $title; ?>" /></label></p>
...
function widget_meta($args) {
	...
	$title = empty($options['title']) ? 'כלים' : $options['title'];
?>
		<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
			<ul>
...
function widget_meta_control() {
	...
	$title = htmlspecialchars($options['title'], ENT_QUOTES);
?>
			<p><label for="meta-title">כותרת: <input style="width: 250px;" id="meta-title" name="meta-title" type="text" value="<?php echo $title; ?>" /></label></p>
...
function widget_calendar_control() {
	...
	$title = htmlspecialchars($options['title'], ENT_QUOTES);
?>
			<p><label for="calendar-title">כותרת:  <input style="width: 250px;" id="calendar-title" name="calendar-title" type="text" value="<?php echo $title; ?>" /></label></p>
...
?>
	<div class="wrap">
		<form method="POST">
			<h2>ווידג'טים מותאמים אישית</h2>
			<p style="line-height: 30px;">אלו ווידג'טים ריקים שאפשר לכתוב בהם ב-HTML או בטקסט רגיל. אפשר לשים תמונות, קטעי טקסט, רשימות, או כל דבר אחר.</p>
			<p>כמה ווידג'טים מותאמים אישית היית רוצה?
			<select id="text-number" name="text-number" value="<?php echo $options['number']; ?>">
<?php for ( $i = 1; $i < 10; ++$i ) echo "<option value='$i' ".($options['number']==$i ? "selected='selected'" : '').">$i</option>"; ?>
			</select>
			<span class="submit"><input type="submit" name="text-number-submit" id="text-number-submit" value="לשמור" /></span></p>
		</form>
	</div>
<?php
}
...
function widget_text_register() {
	...
	if ( $number > 9 ) $number = 9;
	for ($i = 1; $i <= 9; $i++) {
		$name = array('מותאם אישית %s', null, $i);
...
function widget_categories($args) {
	...
	$title = empty($options['title']) ? 'נושאים' : $options['title'];
?>
		<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
			<ul>
...
?>
			<p><label for="categories-title">כותרת:  <input style="width: 250px;" id="categories-title" name="categories-title" type="text" value="<?php echo $title; ?>" /></label></p>
			<p style="text-align:right;margin-right:40px;"><label for="categories-count"><input class="checkbox" type="checkbox" <?php echo $count; ?> id="categories-count" name="categories-count" /> להציג את כמויות הסיפורים</label></p>
			<p style="text-align:right;margin-right:40px;"><label for="categories-hierarchical" style="text-align:right;"><input class="checkbox" type="checkbox" <?php echo $hierarchical; ?> id="categories-hierarchical" name="categories-hierarchical" /> להציג את ההיררכיה</label></p>
...
function widget_recent_comments($args) {
	...
	$title = empty($options['title']) ? 'תגובות אחרונות' : $options['title'];
...
?>
		...
		<?php echo $before_widget; ?>
			...
			echo  '<li class="recentcomments">' . sprintf('%1$s על הפוסט %2$s', get_comment_author_link(), '<a href="'. get_permalink($comment->comment_post_ID) . '#comment-' . $comment->comment_ID . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
			endforeach; endif;?></ul>
		<?php echo $after_widget; ?>
<?php
}
...
function widget_recent_comments_control() {
	...
	$title = htmlspecialchars($options['title'], ENT_QUOTES);
?>
			<p><label for="recent-comments-title">כותרת:  <input style="width: 250px;" id="recent-comments-title" name="recent-comments-title" type="text" value="<?php echo $title; ?>" /></label></p>
...
function widget_recent_comments_register() {
	register_sidebar_widget('תגובות אחרונות', 'widget_recent_comments');
	register_widget_control('תגובות אחרונות', 'widget_recent_comments_control', 300, 90);
...
?>
			<p style="text-align:center;">כתובת הפיד:</p>
			<input style="width: 400px;" id="rss-url-<?php echo "$number"; ?>" name="rss-url-<?php echo "$number"; ?>" type="text" value="<?php echo $url; ?>" />
			<p style="text-align:center;">כותרת הפיד (לא חובה):</p>
			<input style="width: 400px;" id="rss-title-<?php echo "$number"; ?>" name="rss-title-<?php echo "$number"; ?>" type="text" value="<?php echo $title; ?>" />
			<p style="text-align:center; line-height: 30px;">כמה פריטים היית רוצה להציג? <select id="rss-items-<?php echo $number; ?>" name="rss-items-<?php echo $number; ?>"><?php for ( $i = 1; $i <= 10; ++$i ) echo "<option value='$i' ".($items==$i ? "selected='selected'" : '').">$i</option>"; ?></select></p>
...
?>
	<div class="wrap">
		<form method="POST">
			<h2>ווידג'טים עם RSS</h2>
			<p style="line-height: 30px;">הווידג'טים האלו מציגים תכנים מפידים של RSS, לפי בחירתך. אפשר להציג חדשות או עדכונים מבלוגים אחרים, או נתונים משירותי רשת כמו last.fm או flickr, או כל דבר אחר שיש לו פיד RSS.</p>
			<p>כמה ווידג'טים של RSS היית רוצה?
			<select id="rss-number" name="rss-number" value="<?php echo $options['number']; ?>">
<?php for ( $i = 1; $i < 10; ++$i ) echo "<option value='$i' ".($options['number']==$i ? "selected='selected'" : '').">$i</option>"; ?>
			</select>
			<span class="submit"><input type="submit" name="rss-number-submit" id="rss-number-submit" value="לשמור" /></span></p>
		</form>
	</div>
<?php
}
...
register_sidebar_widget('אודות הבלוג', 'widget_description');
register_widget_control('אודות הבלוג', 'widget_description_control', 300, 90);
register_sidebar_widget('עמודים', 'widget_pages');
register_widget_control('עמודים', 'widget_pages_control', 300, 90);
register_sidebar_widget('לוח שנה', 'widget_calendar');
register_widget_control('לוח שנה', 'widget_calendar_control', 300, 90);
register_sidebar_widget('ארכיון', 'widget_archives');
register_widget_control('ארכיון', 'widget_archives_control', 300, 90);
register_sidebar_widget('קישורים', 'widget_links');
register_sidebar_widget('כלים', 'widget_meta');
register_widget_control('כלים', 'widget_meta_control', 300, 90);
register_sidebar_widget('חיפוש', 'widget_search');
register_sidebar_widget('נושאים', 'widget_categories');
register_widget_control('נושאים', 'widget_categories_control', 300, 150);
register_sidebar_widget('פוסטים אחרונים', 'widget_recent_entries');
