...
function kubrick_add_theme_page() {
	...
	add_theme_page('אפשרויות עיצוב: קובריק', 'אפשרויות של העיצוב הנוכחי', 'edit_themes', basename(__FILE__), 'kubrick_theme_page');
}
...
<script type='text/javascript'>
	...
	function PopupWindow_populate(contents) {
		contents += '<br /><p style="text-align:center;margin-top:0px;"><input type="button" value="לסגור את בוחר הצבעים" onclick="cp.hidePopup(\'prettyplease\')"></input></p>';
...
function kubrick_theme_page() {
	if ( $_REQUEST['saved'] ) echo '<div id="message" class="updated fade"><p><strong>השינויים נשמרו.</strong></p></div>';
?>
<div class='wrap'>
	<div id="kubrick-header">
		<h2>תמונה וצבע לכותרת</h2>
		<div id="headwrap">
			<div id="header">
				<div id="headerimg">
		...
		<div id="nonJsForm">
			<form method="POST">
				...
				<label for="njfontcolor">צבע פונט:</label>&rlm;
				<input dir="ltr" type="text" name="njfontcolor" id="njfontcolor" value="<?php echo kubrick_header_color(); ?>" />
				כל צבע CSS&rlm; (<code>red</code>, או <code dir="ltr">#FF0000</code>, או <code dir="ltr">rgb(255, 0, 0)&lrm;</code>&rlm;)</bdo><br />
				
				<label for="njuppercolor">צבע עליון:</label>&rlm;
				<input dir="ltr" type="text" name="njuppercolor" id="njuppercolor" value="#<?php echo kubrick_upper_color(); ?>" />&rlm;
				HEX בלבד (<code dir="ltr">#FF0000</code> or <code dir="ltr">#F00</code>)&rlm;<br />
				
				<label for="njlowercolor">צבע תחתון:</label>&rlm;
				<input dir="ltr" type="text" name="njlowercolor" id="njlowercolor" value="#<?php echo kubrick_lower_color(); ?>" />&rlm;
				HEX בלבד (<code dir="ltr">#FF0000</code> or <code dir="ltr">#F00</code>)&rlm;<br /><br />
				
				<input type="hidden" name="hi" id="hi" value="<?php echo kubrick_header_image(); ?>" />
				
				<label> </label><input type="submit" name="toggledisplay" id="toggledisplay" value="להציג או להסתיר את הטקסט" />
				<input type="submit" name="defaults" value="ברירות מחדל" />
				<input type="submit" class="defbutton" name="submitform" value="&nbsp;&nbsp;לשמור&nbsp;&nbsp;" />
		...
		<div id="jsForm">
			<form style="display:inline;" method="post" name="hicolor" id="hicolor" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
				<input type="button" onclick="tgt=document.getElementById('fontcolor');colorSelect(tgt,'pick1');return false;" name="pick1" id="pick1" value="צבע פונט"></input>
				<input type="button" onclick="tgt=document.getElementById('uppercolor');colorSelect(tgt,'pick2');return false;" name="pick2" id="pick2" value="צבע עליון"></input>
				<input type="button" onclick="tgt=document.getElementById('lowercolor');colorSelect(tgt,'pick3');return false;" name="pick3" id="pick3" value="צבע תחתון"></input>
				<input type="button" name="revert" value="איפוס" onclick="kRevert()" />
				<input type="button" value="עוד אפשרויות" onclick="toggleAdvanced()" />
				<input type="submit" name="submitform" class="defbutton" value="לשמור" onclick="cp.hidePopup('prettyplease')" />
				<input type="hidden" name="action" value="save" />
			...
			<div id="advanced">
				<form id="jsAdvanced" style="display:none;">
					<br />
					<label for="advfontcolor">צבע פונט (CSS):</label>&rlm;
					<input dir="ltr" type="text" id="advfontcolor" onchange="advUpdate(this.value, 'fontcolor')" value="<?php echo kubrick_header_color(); ?>" />&rlm;
					כל צבע CSS&rlm; (<code>red</code>, או <code dir="ltr">#FF0000</code>, או <code dir="ltr">rgb(255, 0, 0)&lrm;</code>&rlm;)</bdo><br />
					
					<label for="advuppercolor">צבע עליון (HEX):</label>&rlm;
					<input dir="ltr" type="text" id="advuppercolor" onchange="advUpdate(this.value, 'uppercolor')" value="#<?php echo kubrick_upper_color(); ?>" />&rlm;
					HEX בלבד (<code dir="ltr">#FF0000</code> or <code dir="ltr">#F00</code>)&rlm;<br />
					
					<label for="advlowercolor">צבע תחתון (HEX):</label>&rlm; 
					<input dir="ltr" type="text" id="advlowercolor" onchange="advUpdate(this.value, 'lowercolor')" value="#<?php echo kubrick_lower_color(); ?>" />&rlm;
						HEX בלבד (<code dir="ltr">#FF0000</code> or <code dir="ltr">#F00</code>)&rlm;<br /><br />
					
					<input type="button" name="default" value="איפוס לברירות המחדל" onclick="kDefaults()" /><br />
					<input type="button" onclick="toggleDisplay();return false;" name="pick" id="pick" value="להציג או להסתיר את הטקסט"></input><br />
				</form>
			</div>
		</div>
	</div>
